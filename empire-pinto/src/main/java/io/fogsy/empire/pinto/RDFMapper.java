...
import io.fogsy.empire.cp.common.utils.base.Option;
import io.fogsy.empire.cp.common.utils.base.Options;
import io.fogsy.empire.cp.common.utils.util.Namespaces;
import io.fogsy.empire.cp.openrdf.utils.model.Statements;
import io.fogsy.empire.cp.openrdf.utils.util.ModelBuilder;
import io.fogsy.empire.cp.openrdf.utils.util.ResourceBuilder;
import io.fogsy.empire.pinto.annotations.Iri;
import io.fogsy.empire.pinto.common.beans.Beans;
import io.fogsy.empire.pinto.common.reflect.Classes;
import io.fogsy.empire.pinto.common.reflect.Fields;
import io.fogsy.empire.pinto.common.reflect.Methods;
import io.fogsy.empire.pinto.util.PintoUtils;
...
public final class RDFMapper {
  ...
  private <T> void addNamespaces(Class<T> theClass) {
    io.fogsy.empire.annotations.Namespaces ns = theClass.getAnnotation(io.fogsy.empire.annotations.Namespaces.class);
    if (ns != null) {
      for (int i = 0; i < ns.value().length - 1; i += 2) {
        mNamespaces.put(ns.value()[i], ns.value()[i + 1]);
      }
    }
  }
  ...
  private boolean isIgnored(final PropertyDescriptor thePropertyDescriptor, Object aInst) {
    // we'll ignore getClass() on the bean
    if (mMappingOptions.is(MappingOptions.IGNORE_PROPERTIES_WITHOUT_ANNOTATION)) {
      if (PintoUtils.getAllFields(new ArrayList<>(), aInst.getClass()).stream().filter(f -> f.getName().equals
              (thePropertyDescriptor.getName())).filter(f -> f.getAnnotation(RdfProperty.class) != null).count() == 0) {
        return true;
      }
    } else {
  ...
  public <T> T readValue(final Model theGraph, final Class<T> theClass, final Resource theObj) {
    ...
    if (theClass.isAssignableFrom(URI.class)) {
      return (T) URI.create(theObj.stringValue());
    }
    ...
    for (PropertyDescriptor aDescriptor : PropertyUtils.getPropertyDescriptors(aInst)) {
      if (isIgnored(aDescriptor, aInst)) {
        continue;
      }

      final IRI aProperty = getProperty(aDescriptor, aInst);
  ...
  private <T> ResourceBuilder write(final T theValue) {
    ...
    try {
      ...
      for (Map.Entry<String, Object> aEntry : PropertyUtils.describe(theValue).entrySet()) {
        ...
        if (isIgnored(aDescriptor, theValue)) {
          continue;
        }

        final IRI aProperty = getProperty(aDescriptor, theValue);
  ...
  private void setValue(final ModelBuilder theGraph, final ResourceBuilder theBuilder,
    ...
    if (Beans.isPrimitive(theObj, mMappingOptions)) {
      ...
      if (serializeCollectionsAsRDFList(thePropertyDescriptor, theObj)) {
  ...
  private boolean serializeCollectionsAsRDFList(final PropertyDescriptor thePropertyDescriptor, Object aInst) {
    if (mMappingOptions.is(MappingOptions.SERIALIZE_COLLECTIONS_AS_LISTS)) {
      return true;
    }

    RdfProperty aProperty = getPropertyAnnotation(thePropertyDescriptor, aInst);
  ...
  private Object valueToObject(final Value theValue, final Model theGraph, final PropertyDescriptor theDescriptor) {
    ...
    } else if (theDescriptor != null && URI.class.isAssignableFrom(theDescriptor.getPropertyType())) {
      return URI.create(theValue.stringValue());
        ...
        if (aTypes[0] instanceof ParameterizedType && ((ParameterizedType) aTypes[0]).getActualTypeArguments().length > 0) {
          ...
          if (aType instanceof Class) {
            ...
            if (mMappingOptions.get(MappingOptions.USE_PROVIDED_CLASSES) == null) {
              ...
              aClass = getClassByAnnotation(theGraph, theResource, annotationProvider);
              if (aClass == null) {
                aClass = (Class) aType;
              }
            }
  ...
  private Class getClassByAnnotation(Model theGraph, Resource theResource, EmpireAnnotationProvider annotationProvider) {
  ...
  private <T> RdfProperty getPropertyAnnotation(final PropertyDescriptor thePropertyDescriptor, T objInstance) {
    Method aMethod = null;
    Field aField = null;
    ...
    if (Methods.annotated(RdfProperty.class).test(thePropertyDescriptor.getReadMethod())) {
      ...
      List<Field> fields = PintoUtils.getAllFields(new ArrayList<>(), objInstance.getClass()).stream().filter(f -> f
              .getName().equals
                      (thePropertyDescriptor.getName())).collect(Collectors.toList());
      if (fields.size() > 0) {
        if (Fields.annotated(RdfProperty.class).test(fields.get(0))) {
          aField = fields.get(0);
        }
      }
    }
    ...
    if (aMethod == null) {
      if (aField != null) {
        return aField.getAnnotation(RdfProperty.class);
      } else {
        return null;
      }
    } else {
  ...
  private IRI getProperty(final PropertyDescriptor thePropertyDescriptor, Object objInstance) {
    final RdfProperty aAnnotation = getPropertyAnnotation(thePropertyDescriptor, objInstance);
  ...
  private <T> Resource id(final T theT) {
    ...
    if (aId == null) {
      List<Field> fields = PintoUtils.getAllFields(new ArrayList<>(), theT.getClass());
      Optional<Field> idField = fields.stream().filter(f -> f.isAnnotationPresent(RdfId.class)).findFirst();
      if (idField.isPresent()) {
        try {
          Field rdfId = idField.get();
          rdfId.setAccessible(true);
          String uri = idField.get().get(theT).toString();
          aId = mValueFactory.createIRI(expand(uri));
